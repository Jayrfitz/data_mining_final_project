...
Our learning curve has a high bias and  
...
```{r}
fit2 = naiveBayes(passingGrade ~ G1 + G2 + reason + address ,data = tr_dat)
```
...
for(tr_size in tr_sizes){
  tr_dat1 = tr_dat[1:tr_size,]
  tr_actual = tr_dat1$passingGrade
  fit2 = naiveBayes(passingGrade ~ G1 + G2 + reason + address, data = tr_dat1)
  # error on training set
...
plot(tr_sizes, tr_errs, type = 'b',lwd = 3, col = 'red', ylim = c(0,0.50))
lines(tr_sizes, te_errs, type = 'b',lwd = 3, col = 'blue')
