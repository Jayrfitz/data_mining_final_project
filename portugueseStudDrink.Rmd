...
Our learning curve has a high bias since our test data error flatens out quickly, while the training data's curve is established immediately. Our graph has underfitting due to a lack of data and features, which do not fit our data well enough. 
...
This model had high bias from the not having enough data, but the error rate was low from using naive bayes. To fix this we will implement more features in the next model down below.
